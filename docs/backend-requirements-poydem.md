...
> Обновление от 26.09.2026: новый handoff фронтенда добавил регистрацию и вход
> по username/password. OAuth остаётся альтернативным способом входа;
> прежняя формулировка ниже про отсутствие password-аутентификации устарела.
...
Основной локальный сценарий использует уникальный без учёта регистра username и
пароль не короче 8 символов; в БД хранится только bcrypt hash. Регистрация создаёт
пользователя с незаполненным профилем. Google, Telegram и VK остаются альтернативными
OAuth-провайдерами. Если профиль не заполнен, frontend открывает onboarding.
...
`lastName`, `avatar`, `about`, `interests`, `gender`, `birthDate`. `gender` принимает
`male`, `female` или `other`, а `birthDate` передаётся как `YYYY-MM-DD` и не может
быть в будущем. До завершения onboarding `city` может
...
Необязательные `minAge` и `maxAge` задаются целыми числами от 14 до 100, причём
`minAge <= maxAge`. Если указана хотя бы одна граница, backend вычисляет возраст по
`birthDate` и проверяет его при открытом вступлении, подаче заявки и повторно перед
approve. Отсутствующая дата рождения или выход за диапазон дают
`403 AGE_RESTRICTION`.
...
При `joinType = open` пользователь вступает без заявки. Backend проверяет: компания `active`, набор открыт, есть место, пользователь не состоит в другой компании этого event, событие доступно, пользователь не banned и проходит возрастное ограничение. Изменения выполняются транзакционно.
...
Перед approve повторно проверяются свободное место, отсутствие пользователя в другой компании этого события, возраст и актуальность компании/события. При конкурентной попытке занять последнее место успешно проходит только одна операция.

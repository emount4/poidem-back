...
`frontend-handoff.md` и фиксирует решения для MVP. Решения перенесены в
канонический [OpenAPI-контракт](../api/openapi.yaml), который используется
для генерации frontend-типов и моков.
...
## Что синхронизировано в OpenAPI

В `api/openapi.yaml` внесены следующие изменения:
...
OpenAPI v1 является единым источником типов и моков для frontend и backend.
Стабильные идентификаторы методов находятся в `operationId`, а точный mapping
ошибок каждой операции — в расширении `x-error-codes`.
